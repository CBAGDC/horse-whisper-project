...
export default {
  ...
  setup() {
    ...
    const fetchArticleData = async (articleId = '18') => {
      try {
        const response = await axios.get('http://cwebcrm.cn:8046/api/v1/notices/article-list?classid=8');
    ...
    const handleTranslateClick = () => {
      router.push({ name: 'zAHone' }); // 跳转到 zn 的 zAHone 页面
    };
