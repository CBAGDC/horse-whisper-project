<template>
  ...
  export default {
    ...
    setup() {
      ...
      const fetchArticleData = async (articleId = '4') => {
        try {
          const response = await axios.get('http://cwebcrm.cn:8046/api/v1/notices/article-list?classid=3');
      ...
      const handleTranslateClick = () => {
        router.push({ name: 'AEquestrian' }); // 跳转到 zn 的 AHorses 页面
      };
