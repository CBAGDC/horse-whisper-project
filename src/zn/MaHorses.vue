<template>
  ...
  <div class="ma-header">
    <p>我和马</p>
    <p :style="{ color: themeStore.isDarkMode ? '#fff' : '#000' }">的故事&nbsp;</p>
  </div>
...
computed: {
  ...
  visibleItems() {
    ...
    return this.items
      .filter(item => item.publishStatus !== -1) // 添加过滤条件
      .slice(start, end);
  }
},
methods: {
  ...
  fetchData() {
    axios.get('http://cwebcrm.cn:8046/api/v1/notices/article-list?classid=1')
      .then(response => {
